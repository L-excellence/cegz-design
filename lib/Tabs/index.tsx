...
const Tabs = ({ defaultActiveKey, items, onChange }: ITabsProps) => {
  ...
  const [inkStyle, setInkStyle] = useState<React.CSSProperties>({ width: 36, left: undefined });
  const [mountInkBar, setMountInkBar] = useState<boolean>(false); // 渲染 Tabs Nav 墨水条
  // 性能优化，记录是否被选中过，初次进入页面，仅渲染 activeKey TabPane
  const activeRecord = useRef<Record<string, boolean>>(items.reduce((pre, cur) => {
        ...
        <div className="cegz-tabs-nav-list">
          {items.map(item => (
            <div 
              ...
              className={classNames("cegz-tabs-tab", { "cegz-tabs-tab-active": activeKey === item.key })}>
              {item.label}
            </div>
          ))}
          {mountInkBar ? (
      ...
      <div className="cegz-tabs-content">
        {items.map(item => (
          activeRecord.current[item.key] ? (
            <div 
              key={item.key} 
              className={classNames("cegz-tabs-tabpane", `cegz-tabs-tabpane-${activeKey === item.key ? "active" : "hidden"}`)}>
              {item.children}
            </div>
          ) : null
        ))}
